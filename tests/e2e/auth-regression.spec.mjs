...
function session(refreshToken = REFRESH) {
  return {
...
async function installAutomaticDesktop(page, { activationError = null } = {}) {
  ...
  await installApiMocks(page, async (request, url) => {
    if (url.pathname !== '/api/auth/desktop/activate') return false;
    if (activationError) {
...
async function installWranglerDesktop(page, { oauthInitiallyReady = true } = {}) {
  await page.addInitScript(({ deviceId, refresh }) => {
    let storedRefresh = null;
    let oauthReady = false;
    let firstBootstrap = true;
    window.__WPAI_TEST_SET_OAUTH__ = value => { oauthReady = value; };
    window.__WPAI_TEST_GET_REFRESH__ = () => storedRefresh;
    window.__TAURI_INTERNALS__ = {
      invoke: async (command, args = {}) => {
        if (command === 'cloudflare_connection_status') {
          return {
    ...
    oauthReady = Boolean(window.__WPAI_OAUTH_INITIAL__);
  }, { deviceId: DEVICE_ID, refresh: REFRESH });
  await page.addInitScript(value => { window.__WPAI_OAUTH_INITIAL__ = value; }, oauthInitiallyReady);
  await installApiMocks(page);
}

async function installApiMocks(page, _unused = null, activationError = null) {
  await page.route('**/*', async route => {
    ...
    if (url.pathname === '/api/auth/desktop/refresh') {
      const body = request.postDataJSON();
      expect(body.deviceId).toBe(DEVICE_ID);
      await route.fulfill(success({ ...session(NEXT_REFRESH), accessToken: `access-${'b'.repeat(80)}` }));
      return;
    }
...
test('an invalid installer activation falls through to Wrangler OAuth rather than asking for Cloudflare credentials', async ({ page }) => {
  await page.addInitScript(({ deviceId, activationToken, refresh }) => {
    let storedRefresh = null;
    window.__TAURI_INTERNALS__ = {
      invoke: async (command, args = {}) => {
        if (command === 'cloudflare_connection_status') return { configured: true, accountId: 'hidden', storage: 'Windows Credential Manager', mode: 'installer_activation', activationToken };
        if (command === 'get_or_create_device_id') return deviceId;
        if (command === 'save_desktop_refresh_token') { storedRefresh = args.token; return null; }
        if (command === 'load_desktop_refresh_token') return storedRefresh;
        if (command === 'remove_desktop_refresh_token') { storedRefresh = null; return null; }
        if (command === 'cloudflare_auto_bootstrap') return { ok: true, mode: 'wrangler_oauth_device', session: { ...session(refresh) } };
        if (command === 'show_desktop_notification') return null;
        throw new Error(`Unexpected desktop command: ${command}`);
      },
      transformCallback: () => 1,
  ...
  await installApiMocks(page, null, 'Bu kurulumun güvenli cihaz etkinleştirmesi geçersiz veya süresi dolmuş.');
  await page.goto('/');
