export type PageId = 'dashboard' | 'whatsapp' | 'contacts' | 'knowledge' | 'files' | 'ai' | 'notifications' | 'reports' | 'settings';
...
export type Health = { ok: boolean; components: Record<string, boolean | string> };
...
export type ConversationDetail = {
  ...
  messages: Array<Record<string, unknown> & { id: string; direction: 'inbound' | 'outbound'; sender_type: string; message_type: string; text_content: string | null; delivery_status: string; ai_generated: number; created_at: string; original_name?: string | null }>;
