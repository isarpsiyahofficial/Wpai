...
-- Production-safe forward migration. This intentionally repeats CREATE IF NOT EXISTS
-- definitions from the restored historical source so deployments that already marked
-- 0004 as applied still receive the complete 376-500 feature schema.
...
CREATE TABLE IF NOT EXISTS ai_training_thread_state (
  thread_id TEXT PRIMARY KEY REFERENCES admin_ai_threads(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_training_thread_state_status ON ai_training_thread_state(status, updated_at DESC);
